            ...
            test_extensions.each do |ext|
              checker = proc {
                begin
                  ...
                  if test_response && status && test_response.content_type != chat.response.content_type && test_response.status_code.to_i < 300 && test_response.has_body?
                    if !test_request.fext.empty? &&  test_response.content_type.match?(/#{test_request.fext}/i)
                  ...
                  if status && test_response.content_type != chat.response.content_type && test_response.status_code.to_i < 300 && test_response.has_body?
                    if !test_request.fext.empty? && test_response.content_type.match?(/#{test_request.fext}/i)
                      addFinding(test_request, test_response,
                                 :test_item => "#{test_request.url}",
                                 :proof_pattern => "#{test_response.status}",
