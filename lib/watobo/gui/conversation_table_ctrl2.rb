  ...
  module Gui
    ...
    class ConversationFilterDialog < FXDialogBox
      ...
      def filter_settings
        ...
        fs[:expression_enabled] = @enable_expression.checked?

        fs[:max_clusters] = @clusters_max.text.to_i
        fs[:clusters_enabled] = @clusters_enabled_cb.checked?
        fs[:num_clusters] = @clusters_num.text.to_i
      ...
      def initialize(owner, filter)
        ...
        matrix = FXMatrix.new(main, 7, :opts => MATRIX_BY_COLUMNS | LAYOUT_FILL_X | LAYOUT_FILL_Y)
        ...
        gb = FXGroupBox.new(matrix, "Ruby", LAYOUT_SIDE_RIGHT | FRAME_GROOVE | LAYOUT_FILL_X | LAYOUT_FILL_Y, 0, 0, 0, 0)
        hframe = FXHorizontalFrame.new(gb, :opts => LAYOUT_FILL_X, :padding => 0)
        frame = FXVerticalFrame.new(gb, :opts => LAYOUT_FILL_X | LAYOUT_FILL_Y, :padding => 0)

        @enable_expression = FXCheckButton.new(hframe, "enable", nil, 0, ICON_BEFORE_TEXT | LAYOUT_SIDE_LEFT)

        @clear_expression_bth = FXButton.new(hframe, "Clear", nil, nil, :opts => BUTTON_NORMAL | LAYOUT_RIGHT)
        @clear_expression_bth.enable
        @clear_expression_bth.connect(SEL_COMMAND) do |sender, sel, item|
          #self.handle(self, FXSEL(SEL_COMMAND, ID_CANCEL), nil)
          @expression.text = ''
        end
        @enable_expression.setCheck !!filter[:expression_enabled]
        FXLabel.new(frame, "Expression,\n e.g. 'chat.request.site =~ /mysite/'")
        #@expression = FXTextField.new(frame, 40, nil, 0, FRAME_SUNKEN|FRAME_THICK|LAYOUT_FILL_X)
        tframe = FXVerticalFrame.new(frame, :opts => LAYOUT_FILL_X | LAYOUT_FILL_Y | FRAME_THICK | FRAME_SUNKEN, :padding => 0)
        @expression = FXText.new(tframe, :opts => LAYOUT_FILL_X | LAYOUT_FILL_Y)
        ...
        #
        #
        #  N E A R E S T   N E I G H B O R   C L U S T E R S
        gb = FXGroupBox.new(matrix, "k-means Clusters (experimental)", LAYOUT_SIDE_RIGHT | FRAME_GROOVE | LAYOUT_FILL_Y, 0, 0, 0, 0)
        vframe = FXVerticalFrame.new(gb, :opts => LAYOUT_FILL_Y, :padding => 0)
        FXLabel.new(vframe, "ML method for detecting outliners.\n If enabled it will preceed all other filters")

        hframe = FXHorizontalFrame.new(vframe, :opts => LAYOUT_FILL_X, :padding => 0)

        @clusters_enabled_cb = FXCheckButton.new(hframe, "enable", nil, 0, ICON_BEFORE_TEXT | LAYOUT_SIDE_LEFT)

        @clusters_enabled_cb.setCheck !!filter[:clusters_enabled]

        #@expression = FXTextField.new(frame, 40, nil, 0, FRAME_SUNKEN|FRAME_THICK|LAYOUT_FILL_X)
        # vframe = FXVerticalFrame.new(frame, :opts => LAYOUT_FILL_X | LAYOUT_FILL_Y | FRAME_THICK | FRAME_SUNKEN, :padding => 0)
        #hframe = FXHorizontalFrame.new(vframe, :opts => LAYOUT_FILL_X, :padding => 0)
        matrix = FXMatrix.new(vframe, 2, :opts => MATRIX_BY_COLUMNS | LAYOUT_FILL_X | LAYOUT_FILL_Y)

        FXLabel.new(matrix, "#clusters to show")
        @clusters_num = FXTextField.new(matrix, 5, nil, 0, FRAME_SUNKEN | FRAME_THICK | LAYOUT_FILL_X)
        @clusters_num.text = filter.has_key?(:num_clusters) ? filter[:num_clusters].to_s : '10'

        #hframe = FXHorizontalFrame.new(matrix, :opts => LAYOUT_FILL_X, :padding => 0)
        FXLabel.new(matrix, "max #clusters")
        @clusters_max = FXTextField.new(matrix, 5, nil, 0, FRAME_SUNKEN | FRAME_THICK | LAYOUT_FILL_X)
        @clusters_max.text = filter.has_key?(:max_clusters) ? filter[:max_clusters].to_s : '2'
      ...
      def initialize(owner, opts)
        ...
        @filter_info.connect(SEL_LEFTBUTTONPRESS) {
          ...
          if dlg.execute != 0
            ...
            if !!@filter[:clusters_enabled]
              puts "Apply k-means cluster filter"
            else
